            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form role="form" id="addTeacherForm" action="{{ url('/dataperkembangan/' . $bulan . '/update') }}" method="post">
                            @csrf
                            <input type="hidden" name="siswa" value="{{$siswa->id}}">
                            <input type="hidden" name="minggu" value="{{$minggu}}">
                            <input type="hidden" name="tahun" value="{{$tahun}}">
                            <div class="card-body">
                                @foreach ($penilaian as $aspek => $per)
                                <label>{{$aspek}}</label>
                                @foreach ($per as $indikasi => $p)
                                <div class="form-group mb-6">
                                    <label for="exampleInputEmail1">{{$loop->iteration}}. {{$indikasi}}</label>
                                    <div class="form-group row">
                                        <div class="col-md-12 mt-1">
                                            <select id="inputSkala" class="form-control" name="skala[{{$p['id']}}]" required>
                                                <option value="1" {{$p['skala'] == '1' ? 'selected': ''}}>1</option>
                                                <option value="2" {{$p['skala'] == '2' ? 'selected': ''}}>2</option>
                                                <option value="3" {{$p['skala'] == '3' ? 'selected': ''}}>3</option>
                                                <option value="4" {{$p['skala'] == '4' ? 'selected': ''}}>4</option>
                                                <option value="5" {{$p['skala'] == '5' ? 'selected': ''}}>5</option>
                                            </select>
                                        </div>
                                    </div>
                                    <input type="text" class="form-control" name="note[{{$p['id']}}]" placeholder="Input Keterangan / Catatan" value="{{$p['catatan']}}" required>
                                </div>
                                @endforeach
                                <br>
                                @endforeach
